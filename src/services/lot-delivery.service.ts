...
async function getIntlThailandStageTypeId(db: Db): Promise<number> {
  ...
  if (!row) throw new Error(`delivery_stage_types missing ${STAGE_INTL_THAILAND_CODE}`)
 ...
 */
export async function markIntlThailandStageDeliveredForLot(lotId: number, tx?: Db): Promise<void> {
  const db = tx ?? prisma
